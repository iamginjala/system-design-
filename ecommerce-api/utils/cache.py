...
def set_data(product_id,my_dict):
    try:
        ...
        logger.debug(f"data stored successfully under {product_id}")
        return True
...
def get_data(product_id):
    try:
        ...
        if raw_data is None or raw_data == "":
            logger.info(f"no data found for key {product_id}")
            return f"no data found for key {product_id}"
        if not isinstance(raw_data, (str, bytes, bytearray)):
            logger.error(f"invalid data type for key {product_id}")
            return f"invalid data type for key {product_id}"
        data = json.loads(raw_data)
        return data
    except json.JSONDecodeError:
        logger.error(f"invalid json data for key {product_id}")
...
def delete_data(product_id):
    try:
        ...
        if deleted:
            ...
            logger.info(f"no data found for {product_id}")
            return f"no data found for {product_id}"

    except Exception as e:
        logger.error(f"No data found for this key {product_id}",exc_info=True)
